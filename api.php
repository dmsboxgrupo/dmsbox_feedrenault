...
function get_content_url( $query, $id = 0, $type = 'image' ) {
	...
	$image_no_crop = has_request('image_no_crop');
	
	if($image_no_crop){ 
	
		return url( "content.php?id={$id}&type={$type}" );
...
switch($q) {
	...
	case 'material_library_tags':
		...
		$meta_property = 'groups_whatsapp_gallery_tags';
		
		if ( $type == 'campaign'){
			
			$meta_property = 'groups_campaign_tags';
			
		} 
		
		$groups_id = get_ids( $db->get_metadata_value( $meta_property ) );
	...
	case 'quick_view_library_tags':
		
		$ids = get_request('ids');
		
		$filter_ids = array_filter( explode(',', $ids) );
		
		$meta_property = 'groups_quick_view_tags';
		
		$groups_id = get_ids( $db->get_metadata_value( $meta_property ) );
	
		$json['groups_quick_view_tags'] = [];
		...
		foreach($groups_id as $group_id) {
			...
			if ($group['status']) {
				...
				if( $ids &&  !in_array( $group_id, $filter_ids)) continue;
				...
				$json['groups_quick_view_tags'][] = $group_item;
...
//print_r($tags);die();			
	...
	case 'quick_views':
		...
		if( $version > 6 ){
			...
			$json['quick_views'] = array();
		
			$id = get_request('id');
			
			//tipo template
			$quick_view_limit = (int)get_request('limit');
			...
			if( $order == 0 ){
				...
				$order_by = " quick_views.date desc, "; 
				
			}else {
				
				if( $order == 1 ){
					
					$order_by = " quick_views.date, "; 
			...
			if( $vehicle != "" ) $quick_view_where .= " and quick_views.vehicle = $vehicle";
			
			if( $id != "" ) $quick_view_where .= " and quick_views.id = $id ";
			
			$tag_group_id = mb_strtolower( get_request('group') );
			...
			if($tag_group_id){
				...
				$w_subgroup = ($tag_subgroup)? " and tag_subgroup=$tag_subgroup " : "";
						$group_arr = array_column($db->query( "SELECT * 
													FROM tags 
													WHERE status=1 and `tag_group` = $tag_group_id $w_subgroup" ), 'id');
		...
		//print_r($group_arr);die();	
			...
			$tags = array();
			$quick_view_tags = array();	
			
			$arr_tag_groups_ids = array();
			$quick_views_db = $db->query("SELECT 
										quick_views.id as id,
										quick_views.date as date,
										quick_views.main_sentence as main_sentence,
										quick_views.name as name,
										quick_views.message as message,
										quick_views.strengths as strengths,
										quick_views.attributes as attributes,
										quick_views.stamps as stamps,
										quick_views.materials as materials,
										quick_views.legal_text as legal_text,
										quick_views.vehicle as vehicle,
										quick_views.image_highlight as image_highlight,
										quick_views.image as image
									FROM `quick_views`
									WHERE status=1 $quick_view_where 
									
									");
									
			//print_r($quick_view_db); die();					
									
			$index_quick_view = 0;
			
			foreach ($quick_views_db as $index_temp => $quick_view_db){
				
				//limitando quantidade de itens retornados
				if( $quick_view_limit> 0 and count($json['quick_views']) >= $quick_view_limit) break;
				
				$item = array();
				
				$quick_view['id'] = (int)$quick_view_db['id'];
				$quick_view['name'] = $quick_view_db['name'];
				$quick_view['image'] = get_content_url( 'image', $quick_view_db['image'] );
				if( $quick_view_db['image_highlight'] )$quick_view['image_highlight'] = get_content_url( 'image', $quick_view_db['image_highlight'] );
				
				if( $quick_view_db['vehicle'] ){	
				
					$vehicle_db = current( $db->query( "SELECT name FROM `vehicles` where status = 1 and `id` = {$quick_view_db['vehicle']} "));
					
					if( $vehicle_db ){
						
						$quick_view['vehicle'] = $quick_view_db['vehicle'];
						$quick_view['vehicle_name'] = $vehicle_db['name'];
				...
				if( $vehicle ||  $id ){
					...
					$quick_view['main_sentence'] = $quick_view_db['main_sentence'];
					$quick_view['message'] = $quick_view_db['message'];
					$quick_view['strengths'] = $quick_view_db['strengths'];
					$quick_view['attributes'] = $quick_view_db['attributes'];
					$quick_view['legal_text'] = $quick_view_db['legal_text'];	
					
					if($quick_view_db['stamps']){
						
						$quick_view['stamps'] = array();
						
						$arr_stamps = array_filter( explode(',', $quick_view_db['stamps']) );
						
						foreach($arr_stamps as $arr_stamp) {
							
							$quick_view_stamp_db = $db->content( $arr_stamp, 'quick_view_stamps' );
							
							$stamp = array();
							$stamp['id'] = (int)$quick_view_stamp_db['id'];
							$stamp['name'] = $quick_view_stamp_db['name'];
							$stamp['text'] = $quick_view_stamp_db['text'];
							//$stamp['image'] = $quick_view_stamp_db['image'];
							$stamp['image'] = get_content_url( 'image', $quick_view_stamp_db['image'] );
								
							array_push ( $quick_view['stamps'],  $stamp);
					...
					if($quick_view_db['materials']){
						
						$quick_view['materials'] = array();
						
						$arr_materials = array_filter( explode(',', $quick_view_db['materials']) );
						rsort($arr_materials);
						
						$index_quick_view_materials = 0;
						
						foreach($arr_materials as $arr_material) {
							
							$quick_view_material_db = $db->content( $arr_material, 'quick_view_materials' );
						//print_r($quick_view_material_db);die();	
							if( $quick_view_material_db['status'] ){
							
								$material = array();
								$material['id'] = (int)$quick_view_material_db['id'];
								$material['name'] = $quick_view_material_db['name'];
								$material['text'] = $quick_view_material_db['text'];
								$material['text_highlight'] = $quick_view_material_db['text_highlight'];
								//$material['image'] = $quick_view_material_db['image'];
								$material['image'] = get_content_url( 'image', $quick_view_material_db['image'] );
						//print_r($quick_view_material_db);		
								$content_type = get_content_format( $quick_view_material_db['image_url'] );
						//print_r($content_type);die();		
								if ($content_type) {
				
									$material['content_type'] = $content_type;
									$material['content'] = url( $quick_view_material_db['image_url'] );
									
								//}
								
								//if (isset($material['content'])) {
				
									$material['download'] = url( get_content_url( '', $quick_view_material_db['image'], '' ) . "&filename={$quick_view_material_db['image']}.{$quick_view_material_db['image_extension']}" );
								
								}
								
								
								if($quick_view_material_db['image_highlight']){
									
									$material['image_highlight'] = get_content_url( 'image', $quick_view_material_db['image_highlight'] );
								
								}
								
								$material['category_id'] = $quick_view_material_db['category'];
								
								//$material['file'] = get_content_url( 'image', $quick_view_material_db['file'] );
								$material['cta'] = $quick_view_material_db['cta'];
								
									///////////////////////////////////////////////////////////////
								
									if($tag_group_id) {
						
										$item_tags = get_ids( $quick_view_material_db['tags'] );
									...
									if($quick_view_material_db['tags']){
						
										$arr_tags = array_filter( explode(',', $quick_view_material_db['tags']) );
									
										$arr_tags_ids = array();
										
										//$arr_template_tags_ids = array();
										
										//print_r($arr_tags);die();
										foreach($arr_tags as $arr_tag) {
											
											if(!in_array( $arr_tag, array_column($tags, 'id'))){								
											
												$quick_view_material_tag = $db->content( $arr_tag, 'tags' );
												
												$tag_item = array();
												$tag_item['id'] = (int)$quick_view_material_tag['id'];
												$tag_item['name'] = $quick_view_material_tag['name'];
												$tag_item['tag_group'] = $quick_view_material_tag['tag_group'];
										
												if(!in_array( $tag_item['tag_group'],$arr_tag_groups_ids))
													array_push ($arr_tag_groups_ids,  $tag_item['tag_group']);		
												
												$tag_item['tag_subgroup'] = $quick_view_material_tag['tag_subgroup'];
												
												$tag_item['materials'][] = $index_quick_view_materials;
												...
												$tags[ $indice ]['materials'][] = $index_quick_view_materials;
										...
										$material['tags'] = implode(",", $arr_tags_ids);
								...
								array_push ( $quick_view['materials'],  $material);
								
								$index_quick_view_materials ++;
							}
						...
						$quick_view['materials'] = "";
					}
					
				}	
				
				$json['quick_views'][] = $quick_view;
			...
			if (!empty($tags)) {
				...
				$arr_quick_view_tags_ids = array();
			
				foreach($quick_view_tags as $quick_view_tag){
					
					//array_push ($arr_quick_view_tags_ids, array_search($quick_view_tag['id'], array_column($tags, 'id')));
					$teste = array_search($quick_view_tag['id'], array_column($tags, 'id'));
					if($teste){
						
						array_push ($arr_quick_view_tags_ids, $teste);
					}
						
				}
					
			}
			//print_r($arr_tag_groups_ids);die();		
			
			if(count($arr_tag_groups_ids)>1){
				$fgroup_ids = implode(",", $arr_tag_groups_ids);
				
				$json['groups']  = get_api_arr("quick_view_library_tags&secret_key=$secret_key&ids=$fgroup_ids", "");
				
			}
			//$json['quick_view_tags'] = implode(",", $arr_quick_view_tags_ids);
	...
	case 'update_user':
		...
		if ( $user )  {
			...
			if( $error == "" && count($user_up) > 0){
				...
				$user_data->update_user_app( $user_up, $nomail );
	...
	//print_r($item['tags']);
				...
				if($filter != "itens_salvos" && (!$search_open_tags)){
					...
					$tags_in_group = array_column($db->query( 
							"SELECT id
								FROM `tags` where 
								`tag_group` > 0 and category<>6 and 
								FIND_IN_SET( `id`, :tagsIds )>0; ", array(
									'tagsIds' => $post['tags']
							) ), 'id');
							//echo "--|";
			...
			/*
			$score_reactions =  $db->query( "SELECT score, 
												CASE 
													WHEN score = 1 THEN 'Love' 
													WHEN score = 2 THEN 'Wow' 
													WHEN score = 3 THEN 'Like' 
													WHEN score = 4 THEN 'Clap' 
													WHEN score = 5 THEN 'Sad' 
													ELSE '' END as name,
													count(*) as cont
													FROM `ranking` 
													WHERE post = :post 
													group by score order by score", array(
					'post' => $post['id']
				) ) ;*/
			...
			if( $item['score_count'] > 0 ){
				...
				$score_reactions = [
						['id' => 1, 'name' => 'Love', 'score_count' => 0],
						['id' => 2, 'name' => 'Wow', 'score_count' => 0],
						['id' => 3, 'name' => 'Like', 'score_count' => 0],
						['id' => 4, 'name' => 'Clap', 'score_count' => 0],
						['id' => 5, 'name' => 'Sad', 'score_count' => 0]
					];
					
				$score_reactions_db =  $db->query( "SELECT score,
													count(*) as cont
													FROM `ranking` 
													WHERE post = :post 
													group by score order by score", array(
				...
				if($score_reactions_db ){
					
					foreach($score_reactions_db as &$score_reaction_db){
						
						$index = $score_reaction_db['score'] - 1;
						$score_reactions[$index]['score_count'] = $score_reaction_db['cont'];
						
					} 
					
				}
				
				$item['score_reactions'] = $score_reactions;
			}
