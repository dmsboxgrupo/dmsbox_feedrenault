			...
			foreach ($quick_views_db as $index_temp => $quick_view_db){
				...
				if( $vehicle ||  $id ){
					...
					if( $quick_view_db['vehicle'] ){	
						...
						$vehicle = current( $db->query( "SELECT name FROM `vehicles` where status = 1 and `id` = {$quick_view_db['vehicle']} "));
						
						if( $vehicle ){
							
							$quick_view['vehicle_name'] = $vehicle['name'];
									...
									if($quick_view_material_db['tags']){
										...
										foreach($arr_tags as $arr_tag) {
											...
											if(!in_array( $arr_tag, array_column($tags, 'id'))){								
												...
												$tag_item['group'] = $quick_view_material_tag['group'];
