...
function makeModal(string, type, sub, timing = true) {
    if(string == 'tour') {
        if($('#tour').hasClass('on')){
            $('#tour').removeClass('ini steps on');
            $('html, body').removeClass('freeze');
...
function filtraVeiculo(target, filtro, sidebar = null){
    if(activeArea == 'materiais-imagens-dos-veiculos'){
        arrFiltros = [];
        window.filtro = arrFiltros;
        ...
        if(!$(target).hasClass('on')) {
            ...
            makeHash('materiais-imagens-dos-veiculos', 'internal', carro)
        }  else {
            $('#lista-filtro .bt-filtro-veiculo').removeClass('on');
        }
    } else {
...
function globalClick(event, target) {
    ...
    switch (true) {
      ...
      case targetClassList.contains('bt-modal'):
        makeModal(btTarget, btMedium, btSub, btTiming);
        break;
...
function montaYoutube(target){
    var thisPlayer = target.parent();
    thisPlayer.find('.hld-youtube .source').html('<div id="playerVideo"></div>');
  ...
  playerVideo = new YT.Player('playerVideo', {
      height: '100%',
      width: '100%',
      videoId: video,
      playerVars: {
