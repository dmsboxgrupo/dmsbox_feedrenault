const LOADING_POST_TEMPLATE =
	'<div class="post" type="feed" reference="loading">' +
	'   <div class="midia">' +
	'       <div class="img"></div>' +
...
const LOADING_SALVOS_TEMPLATE =
	'<div class="post" type="salvos" reference="loading"></div>';

const LOADING_CAMPANHA_TEMPLATE =
	'<div class="post" type="feed" reference="loading">' +
	'   <div class="midia">' +
	'       <div class="img"></div>' +
	'       <div class="description">' +
...
if (loginStatus.check()) {
	if (keeper.get('image') != 'undefined') {
		$('.perfil .dados .img .hld').css({
			'background-image':
				"url('download/img/" + keeper.get('image') + "')",
		});
	} else {
		$('.perfil .dados .img .hld').css({
			'background-image': "url('assets/images/logo_renault.svg')",
		});
	}
...
let hash = [
	...
	{
		internal: 'universo-renault',
		holder: 'dashboard',
		hash: 'universo-renault',
	},
	{
		internal: 'renault-academy',
		holder: 'dashboard',
		hash: 'renault-academy',
	},
	...
	{
		internal: 'meus-itens-salvos',
		holder: 'salvos',
		hash: 'meus-itens-salvos',
	},
	{ internal: 'item-salvo', holder: 'salvos', hash: 'item-salvo' },

	{
		internal: 'biblioteca-de-materiais',
		holder: 'materiais',
		hash: 'biblioteca-de-materiais',
	},
...
function makeHash(string, type, medium, sub) {
	...
	$('.bt-changeview').each(function () {
		...
		if (holder != 'materiais') {
			$('.bt-dropdown[data-target="materiais"]')
				.parent()
				.parent()
		...
		if (holder != 'campanhas') {
			$('.bt-dropdown[data-target="campanhas"]')
				.parent()
				.parent()
				.removeClass('selected');
			$('.bt-dropdown[data-target="campanhas"]')
...
function makePost(page, res, area) {
	if (res.error != null) {
		...
		baseModal +=
			'                <a href="#" class="bt-save" data-sub="galeria">';
		baseModal +=
		...
		baseModal +=
			'                <a href="#" class="bt-close-modal" data-target="modal"></a>';
		baseModal += '            </div>';
		baseModal += '        </div>';
		...
		baseModal += '        <div class="bt">';
		baseModal +=
			'            <a href="#" class="bt-compartilhar">compartilhar</a>';
		baseModal += '        </div>';
		baseModal += '    </div> ';
		baseModal += '</div>';
		...
		if (loopPost) {
				...
				if (item.type != 'enquete') {
					...
					if (item.youtube != undefined && item.youtube != null) {
						...
						htmlCode +=
							...
							'/hqdefault.jpg\');"></div>';
						htmlCode +=
							'       <div class="bt-youtube-close"></div>';
						htmlCode += '   </div>';
						htmlCode +=
							'   <a href="#" class="bt-youtube" data-target="youtube" data-video="' +
							item.youtube +
							'">';
						htmlCode +=
						...
						if (item.content_type == 'video') {
							...
							htmlCode +=
								...
								'\');"></div>';
							htmlCode +=
								'       <div class="bt-video-close"></div>';
							htmlCode += '   </div>';
							htmlCode +=
								'   <a href="#" class="bt-video" data-target="video" data-video="' +
								item.content +
								'">';
							htmlCode +=
							...
							if (item.link != '' && item.link != undefined) {
								htmlCode +=
									'<a href="' +
									item.link +
									'" class="link-externo" target="_blank">';
							} else if (item.pdf != '' && item.pdf != undefined) {
								htmlCode +=
									'<a href="' +
									item.pdf +
									'" class="link-externo" target="_blank">';
								...
								if (item.type == 'campanha') {
									htmlCode +=
										'<a href="#" class="bt-changeview" data-target="' +
										item.origin +
										'" data-sub="' +
										item.id +
										'">';
					...
					htmlCode += '                <div class="aux">';
					htmlCode +=
						'                   <div class="ico"><img src="assets/images/ico_' +
						item.reference +
						'.svg"></div>';
					...
					if (item.reference == 'campanha-momento') {
						let diaExpire = new Date(
							item.expiration_date
						).toLocaleDateString();

						htmlCode += '           <div class="prazo">';
						htmlCode +=
							'               <div class="ico"><img src="assets/images/ico_tempo.svg"></div>';
						htmlCode +=
							'               <div class="txt">válida até <br> <strong>' +
							diaExpire +
							'</strong></div>';
						htmlCode += '           </div>';
					}
					...
					if (loopReaction) {
						loopReaction.forEach(function (react) {
							if (react.id == 1 && react.score_count > 0) {
								htmlCode +=
									'               <img src="assets/images/offset_love.svg">';
							}
							if (react.id == 2 && react.score_count > 0) {
								htmlCode +=
									'               <img src="assets/images/offset_wow.svg">';
							}
							if (react.id == 3 && react.score_count > 0) {
								htmlCode +=
									'               <img src="assets/images/offset_like.svg">';
							}
							if (react.id == 4 && react.score_count > 0) {
								htmlCode +=
									'               <img src="assets/images/offset_clap.svg">';
							}
							if (react.id == 5 && react.score_count > 0) {
								htmlCode +=
									'               <img src="assets/images/offset_sad.svg">';
							}
						});
					}
					htmlCode += '               </div>';
					...
					if (item.user_score > 0) {
						htmlCode +=
							'                    <a href="#" class="bt-react active" data-target="' +
							item.id +
							'" data-type="like">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-react" data-target="' +
							item.id +
							'" data-type="like">';
					}
					...
					if (item.user_score == 3) {
						htmlCode +=
							'                    <a href="#" class="bt-like active" data-target="' +
							item.id +
							'" data-type="like">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-like" data-target="' +
							item.id +
							'" data-type="like">';
					}
					htmlCode +=
					...
					if (item.user_score == 1) {
						htmlCode +=
							'                    <a href="#" class="bt-love active" data-target="' +
							item.id +
							'" data-type="love">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-love" data-target="' +
							item.id +
							'" data-type="love">';
					}
					htmlCode +=
					...
					if (item.user_score == 2) {
						htmlCode +=
							'                    <a href="#" class="bt-wow active" data-target="' +
							item.id +
							'" data-type="wow">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-wow" data-target="' +
							item.id +
							'" data-type="wow">';
					}
					htmlCode +=
					...
					if (item.user_score == 4) {
						htmlCode +=
							'                    <a href="#" class="bt-clap active" data-target="' +
							item.id +
							'" data-type="clap">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-clap" data-target="' +
							item.id +
							'" data-type="clap">';
					}
					htmlCode +=
					...
					if (item.user_score == 5) {
						htmlCode +=
							'                    <a href="#" class="bt-sad active" data-target="' +
							item.id +
							'" data-type="sad">';
					} else {
						htmlCode +=
							'                    <a href="#" class="bt-sad" data-target="' +
							item.id +
							'" data-type="sad">';
					}
					htmlCode +=
					...
					if (item.read_later !== undefined) {
						if (item.read_later) {
							htmlCode +=
								'            <a href="#" class="bt-save active" data-target="' +
								item.id +
								'">';
						} else {
							htmlCode +=
								'            <a href="#" class="bt-save" data-target="' +
								item.id +
								'">';
						}
					}
					...
					htmlCode +=
						'                    <a href="#" class="bt-compartilhar" data-target="' +
						item.id +
						'" data-share-title="' +
						encodeURIComponent(item.title) +
						'" data-share-text="' +
						encodeURIComponent(cleanMessage).replaceAll(
							'_',
							'%20'
						) +
						'" data-share-link="' +
						encodeURIComponent(item.share_link) +
						'">';
					htmlCode +=
					...
					if (item.tell_client != null) {
						htmlCode +=
							'               <div class="hld-bt hld-conte">';
						htmlCode +=
							'                    <a href="#" class="bt-modal" data-target="galeria" data-medium="story" data-sub="' +
							countModal +
							'"  data-download="' +
							item.content +
							'" data-unique="true" data-timing="false" selectable="true">';
						htmlCode +=
							'                       <svg xmlns="http://www.w3.org/2000/svg"  width="29" height="25.7" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 29 25.7">';
						htmlCode +=
							'                         <path class="shape-over" d="M12.7,21s-2.1-2.6-4.5-3.2v-.5c1.3-.6,2.1-2.1,2.1-3.7,0-2.2-1.6-4-3.6-3.9-1.9,0-3.5,1.9-3.4,4.1,0,1.8,1.1,3.3,2.6,3.8v.5c-2.4.9-4.5,3.5-4.5,3.5L.2,25.7h14.3l-1.7-4.7Z"/>';
						htmlCode +=
							'                         <path d="M1.6,25.7H0v-.8c0-4.1,1.5-6.6,4.5-7.5-1.1-.8-1.8-2.1-1.7-3.4-.2-2.3,1.6-4.2,3.8-4.4.2,0,.4,0,.6,0,2.3-.2,4.2,1.5,4.4,3.7,0,.2,0,.4,0,.6,0,1.4-.6,2.6-1.7,3.4,3,.9,4.5,3.4,4.5,7.5v.8h-1.6v-.8c0-4.3-1.8-6.4-5.7-6.4s-5.6,2.1-5.6,6.4v.8ZM10.1,14c.1-1.4-1-2.7-2.4-2.8-.2,0-.3,0-.5,0-1.4-.2-2.7.9-2.9,2.3,0,.2,0,.3,0,.5-.2,1.5.9,2.8,2.3,3,.2,0,.4,0,.6,0,1.5.1,2.8-.9,2.9-2.4,0-.2,0-.4,0-.6M15.8,12.9c-1.9-1.3-3-3.5-2.9-5.8,0-4.4,3.1-7.1,8-7.1s8,2.7,8,7.1-3.1,7.3-8,7.3c-.7,0-1.3,0-2-.2-.9.9-1.9,1.8-2.9,2.6l-1-1.2c1.2-.9,2.4-1.9,3.4-3.1.8.2,1.7.3,2.5.3,4.1,0,6.4-2.1,6.4-5.8s-2.4-5.6-6.4-5.6-6.4,2-6.4,5.6c0,1.8.8,3.6,2.4,4.6-.3.5-.6.9-1.1,1.2"/>';
						htmlCode += '                       </svg>';
						htmlCode += '                    </a>';
						htmlCode += '                    <div class="over">';
						htmlCode +=
							'                        conte para o <br> seu cliente';
						htmlCode += '                    </div>';
						htmlCode += '               </div>';
						...
						if (item.tell_client_type == 'video') {
							htmlModal +=
								'<div class="aux-slide" data-slideID="' +
								item.id +
								'" data-slideSaved="' +
								saved +
								'" data-slideDownload="' +
								item.tell_client +
								'" data-slideTitle="' +
								encodeURIComponent(item.share_message.trim()) +
								'"  data-slideText=""  data-slideLink="' +
								encodeURIComponent(item.tell_client) +
								'"  data-vehicle="' +
								item.vehicles +
								'" data-version="' +
								item.version +
								'">';
							htmlModal +=
								'  <div class="slide sld-video autoplay" style="background-image:url(\'' +
								item.content +
								'\');">';
							...
							htmlModal += '      </div>';
							htmlModal +=
								'      <a href="#" class="bt-video-modal" data-target="video" data-video="' +
								item.tell_client +
								'">';
							htmlModal +=
								'          <div class="img" style="background-image: url(\'' +
			...
			let actualArea = window.location.hash.substring(
				1,
			...
			$('.hld-loading-' + page).remove();
			$('#internal-holder').append(
				'<div class="txt-fim">Isso é tudo! &nbsp;&nbsp;&nbsp; :)</div>'
			);
		}
	}
}
...
let arrTourDesktop = [
	{
		target: '.switcher [data-target="perfil"]',
		boxsize: 'box-p',
		offset: '-1,2,0,4',
		boxposition: 'right',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: 'border-radius:30px;',
		text: 'Encontre configurações e mais opções nesse menu.',
	},
	{
		target: '.accordion .master-bt:nth-child(1)',
		boxsize: 'box-p',
		offset: '10,0,10,0',
		boxposition: 'right',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: 'border-radius:30px;',
		text: 'Clique aqui para voltar à página inicial a qualquer momento.',
	},
	{
		target: '.accordion .master-bt:nth-child(2)',
		boxsize: 'box-p',
		offset: '10,0,10,0',
		boxposition: 'right',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: 'border-radius:30px;',
		text: 'Encontre todos os materiais para download e compartilhamento nesta biblioteca.',
	},
	{
		target: '.accordion .master-bt:nth-child(3)',
		boxsize: 'box-p',
		offset: '10,0,10,0',
		boxposition: 'right',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: 'border-radius:30px;',
		text: 'Acesse as campanhas mais recentes e fique por dentro das novidades.',
	},
	{
		target: '.menu-slider.destaque',
		boxsize: 'box-m',
		offset: '0,10,10,10',
		boxposition: 'bottom',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Confira os conteúdos em destaque e informações mais importante.',
	},
	{
		...
		tipposition: '',
		ajust: '',
		text: 'Veja os conteúdos por temas e organize a ordem de acordo com sua preferência.',
	},
	{
		target: '#internal-holder .post:nth-child(1)',
		boxsize: 'box-g',
		offset: '0,16,16,16',
		...
		tipposition: 'left',
		ajust: '',
		text: 'Leia os conteúdos completos e esteja sempre atualizado.',
	},
	{
		target: '#internal-holder .post:nth-child(1) .bt-react',
		boxsize: 'box-m',
		offset: '8,8,8,8',
		...
		tipposition: 'right',
		ajust: '',
		text: 'Gostou? Toque para reagir e sabermos o que achou!',
	},
	{
		target: '#internal-holder .post:nth-child(1) .hld-reactions .over',
		boxsize: 'box-m',
		offset: '0,8,0,0',
		boxposition: 'top-right',
		offsetbox: '0,0,0,0',
		tipposition: 'right',
		ajust: '',
		text: 'Escolha sua reação: curta, ame, aplauda, entre outras.',
	},
	{
		target: '#internal-holder .post:nth-child(1) .bt-save',
		boxsize: 'box-m',
		offset: '8,8,8,8',
		boxposition: 'top-right',
		offsetbox: '0,0,0,0',
		tipposition: 'right',
		ajust: '',
		text: 'Salve este conteúdo para encontrá-lo facilmente depois.',
	},
	{
		target: '.area .bt',
		boxsize: 'box-p',
		offset: '5,5,5,5',
		boxposition: 'left-top',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Personalize seu Feed: para escolher a prioridade de conteúdos, clique aqui.',
	},
	{
		target: '.panel-config',
		boxsize: 'box-g',
		offset: '',
		boxposition: 'left-center',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Organize os temas e a ordem em que deseja vê-los, de acordo com sua preferência.',
	},
	{
		target: 'aside.right .banners',
		boxsize: 'box-m',
		offset: '8,8,8,8',
		boxposition: 'left-top',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Confira os principais destaques aqui e acesse os materiais facilmente.',
	},
	{
		target: '.bt.informacao',
		boxsize: 'box-m',
		offset: '-1,2,0,4',
		...
		tipposition: 'right',
		ajust: '',
		text: 'Quando houver qualquer dúvida esse tutorial estará sempre disponível aqui. ',
	},
	{
		target: '.bt.notificacao',
		boxsize: 'box-m',
		offset: '-1,2,0,4',
		boxposition: 'bottom-right',
		offsetbox: '0,0,0,0',
		tipposition: 'right',
		ajust: '',
		text: 'Veja os conteúdos mais recentes e os que ainda não foram visualizados.',
	},
	{
		target: '.bt.busca',
		boxsize: 'box-m',
		...
		tipposition: 'right',
		ajust: '',
		text: 'Use a busca para encontrar rapidamente conteúdos específicos.',
	},
	{
		target: '.menu-slider.icons',
		boxsize: 'box-m',
		offset: '0,10,10,10',
		boxposition: 'bottom',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Navegue pelos diferentes tipos de materiais disponíveis para download e compartilhamento.',
	},
	{
		target: '#sidebar-holder',
		boxsize: 'box-p',
		offset: '-74,0,0,0',
		boxposition: 'left-center',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Utilize o filtro por veículo para encontrar rapidamente os materiais desejados.',
	},
	{
		target: '#internal-holder .post:nth-child(1)',
		boxsize: 'box-p',
		...
		tipposition: '',
		ajust: '',
		text: 'Ao clicar em um material ou passar o mouse por ele, você pode baixar ou compartilhar o conteúdo diretamente com seu cliente.',
	},
	{
		target: '.menu-slider.icons',
		boxsize: 'box-m',
		...
		tipposition: '',
		ajust: '',
		text: 'Para encontrar rapidamente os materiais de campanha que estão disponíveis, utilize o filtro de acordo com o tema desejado.',
	},
	{
		target: '#internal-holder .type-campanha-momento .title',
		boxsize: 'box-p',
		offset: '10,10,10,10',
		boxposition: 'bottom',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'As campanhas disponíveis podem ser fixas ou com data de validade. As que possuírem validade serão sinalizadas com este ícone e a respectiva data.',
	},
	{
		target: '#internal-holder .type-campanha-momento .description',
		boxsize: 'box-p',
		offset: '1,17,17,0',
		boxposition: 'right',
		offsetbox: '0,0,0,0',
		tipposition: '',
		ajust: '',
		text: 'Acesse todos os materiais das campanhas fixas clicando diretamente no link do Salesforce.',
	},
];
...
let arrTourMobile = [
	...
	{
		target: '.nav-mobile .master-bt:nth-child(1)',
		boxsize: 'box-mobile',
		offset: '1,2,1,2',
		boxposition: 'mobile-top',
		offsetbox: '0,0,0,0',
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Clique aqui para voltar à página inicial a qualquer momento.',
	},
	{
		target: '.nav-mobile .master-bt:nth-child(2)',
		boxsize: 'box-mobile',
		offset: '1,2,1,2',
		boxposition: 'mobile-top',
		offsetbox: '0,0,0,0',
	...
	{
		target: '.nav-mobile .master-bt:nth-child(3)',
		boxsize: 'box-mobile',
		offset: '1,2,1,2',
		boxposition: 'mobile-top',
		offsetbox: '0,0,0,0',
	...
	},
	{
		...
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Clique para reagir ao conteúdo e nos contar o que achou!',
	},
	{
		target: '#internal-holder .post:nth-child(1) .hld-reactions .over',
		boxsize: 'box-mobile',
		...
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Escolha sua reação: curta👍, ame❤️, aplauda👏, entre outras.',
	},
	{
		target: '#internal-holder .post:nth-child(1) .bt-save',
		boxsize: 'box-mobile',
		...
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Salve este conteúdo para encontrá-lo facilmente depois. Para isso, clique nas três barras no canto superior direito. ',
	},
	{
		target: '.bt-config',
		boxsize: 'box-mobile',
		...
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Personalize seu Feed: clique aqui e defina a ordem e os assuntos que mais te interessam.',
	},
	{
		target: '.panel-config .configs',
		boxsize: 'box-mobile',
		offset: '-82,32,-24,0',
		boxposition: 'mobile-bottom',
		offsetbox: '80,0,0,0',
	...
	},
	{
		...
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Veja os conteúdos e notificações mais recentes que ainda não foram visualizados.',
	},
	{
		target: '.nav-mobile .master-bt:nth-child(4)',
		boxsize: 'box-mobile',
		offset: '',
		boxposition: 'mobile-top',
		offsetbox: '0,0,0,0',
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Use a busca e encontre rapidamente os materiais e conteúdos que você deseja.',
	},
	{
		target: '.nav-mobile .master-bt:nth-child(5)',
		boxsize: 'box-mobile',
		offset: '',
		boxposition: 'mobile-top',
		offsetbox: '0,0,0,0',
		tipposition: 'mobile-tip',
		ajust: '',
		text: 'Está com dúvidas? Não se preocupe, nosso tutorial ficará disponível aqui.',
	},
	{
		target: '.menu-slider.icons',
		boxsize: 'box-mobile',
	...
	{
		target: '.filtro-mobile .bt-filtro',
		boxsize: 'box-mobile',
		offset: '0,0,0,0',
		boxposition: 'mobile-bottom',
		offsetbox: '0,0,0,0',
...
function showStep(num, direction) {
		...
		if (num == 20) {
			if (direction == 'next') {
				$('html, body').animate(
					{
						scrollTop:
							$('.type-campanha-momento').offset().top - 50,
					},
					400
				);
				waitStep = 500;
		...
		if (num == 20) {
			if (direction == 'next') {
				$('html, body').animate(
					{
						scrollTop:
							$('.type-campanha-momento').offset().top - 100,
					},
					400
				);
				waitStep = 500;
	...
	let waitGeral = setTimeout(function () {
		...
		if (offsetShape) {
			...
			newStyles =
				'top:' +
				(position.top - offsetArray[0]) +
				'px; left:' +
				(position.left - offsetArray[3]) +
				'px; width:' +
				($(item).width() + offsetArray[1] * 2) +
				'px; height:' +
				($(item).height() + offsetArray[2] * 2) +
				'px;';
		} else {
			newStyles =
				'top:' +
		...
		if (tipPosition == 'mobile-tip') {
			...
			$('head').append(
				'<style>#box-tour:before{left:' + posTip + 'px;}</style>'
			);
		}
			...
			if (offsetShape) {
				$('#box-tour').attr(
					...
					'top:' +
						(position.top - offsetArray[0]) +
						'px; left:' +
						(position.left - $('#box-tour').width() - 75) +
						'px;'
				);
			} else {
				$('#box-tour').attr(
		...
		if (boxPosition == 'left-center') {
			if (offsetShape) {
				$('#box-tour').attr(
					'style',
					'top: 50%; transform: translate(0,-50%); left:' +
						(position.left - $('#box-tour').width() - 75) +
						'px;'
				);
			} else {
		...
		if (boxPosition == 'top-center') {
			$('#box-tour').attr(
				'style',
				'top:' +
					(position.top - $('#box-tour').height() - 55) +
					'px; left:' +
					(position.left + $('.tour .shape').width() / 2) +
		...
		if (boxPosition == 'top-right') {
			$('#box-tour').attr(
				'style',
				'top:' +
					(position.top - $('#box-tour').height() - 60) +
					'px; left:' +
					(position.left - $('#box-tour').width() + 10) +
					'px;'
			);
		}
		if (boxPosition == 'bottom') {
			$('#box-tour').attr(
				'style',
				'top:' +
					(position.top + $('.tour .shape').height() + 30) +
			...
			if ($(window).width() < 992) {
				$('#box-tour').attr(
					...
					'top:' +
						(position.top + $('.tour .shape').height() + 30) +
						'px; left:' +
						(position.left - $('#box-tour').width() + 5) +
						'px;'
				);
			} else {
				$('#box-tour').attr(
					...
					'top:' +
						(position.top + $('.tour .shape').height() + 30) +
						'px; left:' +
						(position.left - $('#box-tour').width() + 20) +
						'px;'
				);
			}
		}
		...
		if (boxPosition == 'mobile-bottom') {
			$('#box-tour').attr(
				...
				'top:' +
					(position.top +
						Number(offsetBoxArray[0]) +
						($('.tour .shape').height() + 30)) +
					'px; left: 16px;'
			);
		}
		...
		if ($(window).width() < 992) {
			let wait = setTimeout(function () {
				...
				$('.nav-mobile .bt:not(.selected)').css({
					'pointer-events': 'none',
				});
			}, 25);
		}
...
function salvaDados(type) {
	...
	if (type == 'password') {
		...
		let senhaConfirma = $(
			'.configuracao input[name="confirma-senha"]'
		).val();

		$('.msg-error').removeClass('on');
		$('.configuracao input[name="senha-atual"]')
			.parent()
			.removeClass('error');
		$('.configuracao input[name="nova-senha"]')
			.parent()
			.removeClass('error');
		$('.configuracao input[name="confirma-senha"]')
			.parent()
	...
	if ($('#' + string + ' .slider').length > 0) {
		...
		slider.on(
			...
			function (
				...
				if (
					$('*[data-slick-index="' + currentSlide + '"] .playing')
						.length > 0
				) {
				...
				if (
					...
					var thisPlayer = $(
						'*[data-slick-index="' + nextSlide + '"] .sld-video'
					);
					var video = $(
						'*[data-slick-index="' +
							nextSlide +
				...
				let slideID = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideID');
				let slideSaved = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideSaved');
				let slideDownload = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideDownload');
				let slideTitle = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideTitle');
				let slideText = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideText');
				let slideLink = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-slideLink');

				let slideVehicle = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-vehicle');
				let slideVersion = $('*[data-slick-index="' + nextSlide + '"]')
					.find('.aux-slide')
					.attr('data-version');
				...
				if (slideID) {
					slider
						.parent()
						.find('.bt-save')
						.attr('data-target', slideID);
					slider
						.parent()
						.parent()
						.find('.bt-baixar')
						.attr('data-target', slideDownload);
					slider
						.parent()
						.parent()
						.find('.bt-baixar')
						.attr('data-medium', slideID);
					slider
						.parent()
						.parent()
						.find('.bt-baixar')
						.attr('data-vehicle', slideVehicle);
					slider
						.parent()
						.parent()
						.find('.bt-baixar')
						.attr('data-version', slideVersion);
					slider
						.parent()
						.parent()
						.find('.bt-compartilhar')
						.attr('data-target', slideDownload);
					slider
						.parent()
						.parent()
						.find('.bt-compartilhar')
						.attr('data-share-title', slideTitle);
					slider
						.parent()
						.parent()
						.find('.bt-compartilhar')
						.attr('data-share-text', slideText);
					slider
						.parent()
						.parent()
						.find('.bt-compartilhar')
						.attr('data-share-link', slideLink);
					slider
						.parent()
						.parent()
						.find('.bt-compartilhar')
		...
		slider.on(
			'afterChange',
			function (event, currentSlide, currentSlideIndex) {
				let id = $('.slick-current').find('.aux-scroll').attr('id');
				let pdf = $('.slick-current')
					.find('.aux-scroll')
					.attr('data-pdf');
		...
		slider.on('init', function (event, currentSlide, currentSlideIndex) {
			let wait = setTimeout(function () {
				...
				let slideID = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideID');
				let slideSaved = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideSaved');
				let slideDownload = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideDownload');
				let slideTitle = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideTitle');
				let slideText = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideText');
				let slideLink = $('.slick-current')
					.find('.aux-slide')
					.attr('data-slideLink');

				let slideVehicle = $('.slick-current')
					.find('.aux-slide')
					.attr('data-vehicle');
				let slideVersion = $('.slick-current')
					.find('.aux-slide')
					.attr('data-version');
		...
		if (timing == 'false') {
			...
			setInterval(function () {
				$('.modal li.slick-active button').attr(
					'style',
					'width:' + window.slickTime + '%'
				);
			}, 50);
		}
...
function share(target, id) {
	...
	let emailText = decodeURIComponent(text)
		.replace(/<p[^>]*>/g, '')
		.replace(/<\/p>/g, '%0D%0A');
	...
	$('.modal.compartilhar')
		.find('[data-type="whatsapp"]')
		.attr('href', whatsappURL);
	$('.modal.compartilhar')
		.find('[data-type="telegram"]')
		.attr(
			'href',
			'https://t.me/share/url?url=' + link + '&text=' + emailText
		);
	$('.modal.compartilhar')
		.find('[data-type="facebook"]')
		.attr(
			'href',
...
function savePost(target, id, sub = null) {
	...
	if ($(target).hasClass('active')) {
		...
		if (sub == 'galeria') {
			$('#galeria .slick-current')
				.find('.aux-slide')
				.attr('data-slideSaved', 'false');
		}
	} else {
...
function reactPost(target, id) {
	let params = '';
	let statusInicial = $('.bt-react[data-target="' + id + '"]').hasClass(
		'active'
	);
	let hldID = id;
		...
		if (res.error != null) {
			...
			app.request(paramsCheck, false, null, (res) => {
				if (res.error != null) {
					...
					let item = res.feed[0];
					$('.bt-react[data-target="' + hldID + '"]')
						.parent()
						.parent()
						.parent()
						.find('.reactions')
						.attr('data-num', item.score_reactions);
					let htmlCode = '';
					let loopReaction = item.score_reactions;
					...
					if (item.score_count > 0) {
						if (item.score_count > 1) {
							$('.bt-react[data-target="' + hldID + '"]')
								...
								.parent()
								.parent()
								.find('.reactions .status')
								.text(item.score_count + ' reações');
						} else {
							$('.bt-react[data-target="' + hldID + '"]')
								.parent()
								.parent()
...
function scroll(target) {
	...
	$('html, body').animate(
		{
			scrollTop:
				$('*[data-anchor="' + target + '"]').offset().top - offset,
		},
		1000
	);
}
...
function filtraVeiculo(target, filtro, sidebar = null) {
	if (
		activeArea == 'materiais-imagens-dos-veiculos' ||
		activeArea == 'materiais-ficha-de-modelos'
	) {
		arrFiltros = [];
		window.filtro = arrFiltros;
...
function globalClick(event, target) {
	...
	console.log(
		event + ' | ' + target + ' | ' + targetClassList + ' | ' + btTarget
	);
...
function montaYoutube(target) {
	...
	var thisPlayer = target.parent();
	thisPlayer
		.find('.hld-youtube .source')
		.html('<div id="playerVideo-' + video + '"></div>');
...
async function carregaPDF(url, containerId) {
	const container = $('#' + containerId);
	if (!container.length || !container.is(':visible')) {
		return;
	}

	// PONTO 1: Cancela a operação anterior, se houver.
	// Isso interrompe qualquer getDocument ou renderização que ainda esteja em andamento.
	if (currentPdfController) {
		currentPdfController.abort();
	}

	// PONTO 2: Cria um novo controlador para a operação ATUAL.
	const controller = new AbortController();
	currentPdfController = controller;

	container
		.empty()
		.html('<div class="loader-panel"><div></div><span>0</span></div>');

	const pdfjsLib = window['pdfjs-dist/build/pdf'];
	pdfjsLib.GlobalWorkerOptions.workerSrc = '../libraries/pdf.worker.min.js';

	try {
		// PONTO 3: Passa o "sinal" do controlador para o PDF.js.
		// Agora, se chamarmos controller.abort(), esta operação será interrompida.
		...
		loadingTask.onProgress = function (data) {
			const pct = Math.floor((99 * data.loaded) / data.total);
			container.find('.loader-panel > span').text(pct);
		};

		const pdf = await loadingTask.promise;

		// Se o sinal foi abortado enquanto esperávamos pelo PDF, paramos aqui.
		...
		for (let i = 1; i <= numPages; i++) {
			pagesHTML += `<div class="pdfpage" data-page-number="${i}"><canvas class="pdfcanvas"></canvas><div class="annotation"></div></div>`;
		}
		container.append(pagesHTML);

		for (let pageNum = 1; pageNum <= numPages; pageNum++) {
			// A cada iteração, verificamos se a tarefa foi cancelada.
			if (controller.signal.aborted) return;

			const page = await pdf.getPage(pageNum);
			const viewport = page.getViewport({ scale: 1.5 });

			const pageElement = container.find(
				`.pdfpage[data-page-number="${pageNum}"]`
			);
			if (!pageElement.length) continue;
			...
			// A task de renderização também respeitará o sinal de cancelamento.
			await page.render(renderContext).promise;

			pageElement.addClass('loaded');
			await setupAnnotations(page, pageElement.find('.annotation'));
		}

		container.find('.loader-panel').addClass('loader-inactive');
	} catch (reason) {
		// PONTO 4: Tratamos o erro de cancelamento de forma silenciosa.
		if (
			reason.name === 'AbortException' ||
			reason.name === 'CancellationException' ||
			reason.name === 'AbortError'
		) {
			console.log('Tarefa de carregamento de PDF cancelada com sucesso.');
		} else {
			console.error('Erro ao carregar o PDF:', reason);
			container.find('.loader-panel').text('Erro ao carregar o PDF.');
		}
	} finally {
		// PONTO 5: Limpamos o controlador se ele for o que acabou de terminar.
		if (currentPdfController === controller) {
	...
	var promise = page.getAnnotations().then(function (annotationsData) {
		...
		for (var i = 0; i < annotationsData.length; i++) {
			...
			var el =
				...
				"style='" +
				'transform: scale(' +
				annotationLayerDiv.outerWidth() / v[2] +
				'); ' +
				'transform-origin:' +
				-r[0] +
				'px ' +
