...
//echo "mail_team_incentive: $mail_team_incentive"; //die();
	...
	public function update_user_app($user, $nomail = 0){
		...
		if (!array_key_exists("background_image", $user) || count($user) != 1){
			...
			if(!$nomail)$this->enviaContaAlterada( $this->name, $this->email, "app");
	...
	public function enviaPendenciaGerente($usuario_novo, $id_usuario, $bir_usuario, $email_usuario){
		...
		//$link = HOST."/#login-confirmacao&token=".$uuid;
	...
	//print_r($level);die();	
		...
		if($level == 2){
			...
			$link = HOST."/#login-cadastro-senha&token=".$uuid;
	...
	public function enviaConfirmacao($usuario_novo, $email_destino, $origem="web"){
		...
		if($this->origin==1){
			...
			//$link = HOST."/#login-confirmacao&token=".$uuid;
			$link = "https://feedrenault.com.br/#login-confirmacao&token=".$uuid;
			...
			$link = HOST."/#login-confirmacao-email&token=".$uuid;
			*/
			...
			//$link = HOST."/#login-confirmacao-email&token=".$uuid;
			$link = "https://feedrenault.com.br/#login-confirmacao-email&token=".$uuid;
	...
	public function enviaDesativacao($usuario_novo, $email_destino, $origem){
		...
		//$link = HOST."/#request_reactivation&token=".$uuid;
		$link = HOST."/#login-confirmacao-gerente&token=".$uuid;
	...
	public function enviaReforcoAtivacao1($usuario_novo, $email_destino, $origem="web"){
		...
		$link = HOST."/#login-confirmacao-email&token=".$uuid;
		//$link = "localhost/feedrenault_webapp/#login-confirmacao-email&token=".$uuid;
