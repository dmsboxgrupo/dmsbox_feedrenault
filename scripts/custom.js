...
$(function() {
  ...
  $('body').on('click', '.bt-fechar-geral', function(ev) {
        ...
        if($(this).hasClass('clear-card')){
          $('#cardpopup').html('');
        }
  });

  $('body').on('click', '.bt-fundo', function(ev) {
         ev.preventDefault();
         ev.stopImmediatePropagation();
         $('.bt-fundo').removeClass('selected');
  ...
  $('.popup-page').each(function() {
    ...
    if (!bodyEvents) $("body").on("click", "#menu-subsecoes a", function() {
      if($(this).attr('data-filter') == 'compartilhar') {
        $('#subsecao').addClass('compartilhar');
      } else {
        $('#subsecao').removeClass('compartilhar');
      }
    ...
    if (!bodyEvents) document.addEventListener('wheel', function(e) {
      ...
      if (e.target.closest("#menu-desktop") || e.target.id == "desktop-container" || e.target.id == "header-desktop") {
		...
		if ( $(".proxy-scroll").length > 1 ) $(".proxy-scroll")[1].scrollTop -= e.wheelDelta;
		else if ( $(".proxy-scroll").length > 0 ) $(".proxy-scroll")[0].scrollTop -= e.wheelDelta;
        e.preventDefault(); // Prevent user scroll during page jump
      }
    }, {passive: false});
...
*/
	...
	if (!bodyEvents) $('body').on('click', '.curtir', function(ev) { // Like
	  ...
	  var ativo = $(this).hasClass('ativo');
	  var score_val = $(this).next().find("[data-custom='score-count']").text();
	  ...
	  if( score_val > 1 ){
		...
		$(this).next().html("<span><span data-custom='score-count'>" + score_val + "</span> Curtidas</span>");
	  
	  } else {
		  
		  if( score_val == 1 ){
			
			$(this).next().html("<span><span data-custom='score-count'>1</span> Curtida</span>");
			
		  } else {
		  
			$(this).next().html("<span><span></span> Curtir</span>");
    ...
    */
    ...
    // Editar Post NOVO ###
    if (!bodyEvents) $('body').on('click', '.ico-editar', function(ev) {
      ev.stopImmediatePropagation();
    ...
    if (!bodyEvents) $('body').on('click', '.img-editar', function(ev) {
      ev.stopImmediatePropagation();
      var cardId = $(this).attr('data-id')
      ...
      // Download Post
    if (!bodyEvents) $('body').on('click', '.compartilhar .acao-icone.download', function(ev) {
      ev.stopImmediatePropagation();
      signal.emit('download', { content: $(this).parent().parent().parent() });
    });
    ...
    if (!bodyEvents) $('body').on('click', '.link-interna', function(ev) {
      ...
      if($(this).attr('data-subinterna')) {
        $('.header-interna .interna-fechar').addClass('subinterna');
      }

      if($(this).attr('data-bt') == 'close') {
        $('.header-interna').addClass('ico-close');
        $('.header-interna').removeClass('ico-back');
      } else {
        $('.header-interna').addClass('ico-back');
        $('.header-interna').removeClass('ico-close');
      }
    });

    if (!bodyEvents) $('body').on('click', '.interna-hide', function(ev) {
      $('.header-interna').find('.header-title').html('');
      $('.header-interna').removeClass('on');
    });

    if(app.urlParams.p) {
      if (app.urlParams.p.substring(0,6) != 'create') {
    ...
    if (!bodyEvents) $('body').on('click', '.interna-fechar', function(ev) {
      /*if(!$(this).hasClass('subinterna')) {
        $('.header-interna').find('.header-title').html('');
        $('.header-interna').removeClass('on');
      }*/
      if($(this).hasClass('back-create-home')) {
        location.hash = "p=create";
      }else if($(this).hasClass('back-create')) {
        location.hash = "p=create-home&t=" + app.urlParams.t;
      } else if($(this).hasClass('back-home')) {
        location.hash = "p=home";
      } else {
        $(this).removeClass('subinterna');
        history.back();
      }
    });
	...
	if (!bodyEvents) $('body').on('click', '[content-action=proxleitura]', function(ev) {
      ev.stopImmediatePropagation();
	  popupConfirmacao("Deseja retirar esse conteúdo das suas próximas leituras?", function() {
	  });
    });
    ...
    function fechaPesquisa() {
      if ($("#pesquisa").hasClass("visivel")) {
        ...
        $('#page').removeClass('pesquisa-on');
        $('.header-title').text('');

          $("#pesquisa").removeClass("visivel");
          $("#pesquisa .pesquisa-topo input").val('');
    ...
    signal.on('page_complete', arg => arg.page == 'home.html' ? fechaPesquisa() : '');
    signal.on('params', arg => arg.p == 'pesquisa' && arg.q == null ? limpaPesquisa() : '');
    ...
    if (!bodyEvents) {
      ...
      $("body").on("click", ".ordenar .dropdown a", function(ev) {
        ...
        $('*[name="inp-order"]').val($(this).html());
        signal.emit('filtrar', { value: $(this).html() });
      });
    ...
    if (!bodyEvents) $("body").on("click", ".area-fechar", function(ev) {
      hideMenu();
    });
    // Configuracoes
	...
	function destaqueMostraConte(indice, quantos, naoAvanca, DestaqueInferior = null, salva = null, id = null) {
		
      if (indice >= quantos) indice = quantos - 1;
      var itens = "";
      var popup = $("#destaque-popup");
      var container = $("#destaque");
	    var container_item = $("#destaque .destaque-item");
		
      $("#destaque .destaque-barra, #destaque .destaque-fecha, " + 
      "#destaque .destaque-anterior, #destaque .destaque-proximo").remove();
	  
	   if(!salva) {
  	   $("#destaque .destaque-inferioresq").remove();
     }
	   $("#destaque .destaque-inferiordir").remove();


     if(salva == '1') {
      $("#destaque .destaque-inferioresq").find('.proxleitura').addClass('ativo');
     }
     if(id) {
      $("#destaque").attr('content-id', id);
     }

	   var container_item = $("#destaque .destaque-item");
	   container_item.addClass('ativo');
	   
	   if(DestaqueInferior) {
	   
		container_item.append(DestaqueInferior);
		
	   }
	   /*var DestaqueInferior = document.createElement('div');
			
		$(DestaqueInferior).attr({'class': 'destaque-inferior'});
		
		var Content = document.createElement('a');
			
		$(Content).attr({
					  'content-id': 3002,
					  'content-url': 'http:\/\/localhost\/renault\/uploads\/5285.png?v=2',
					  'content-download': 'http:\/\/localhost\/renault\/uploads\/5290.pdf',
					  'class': 'acao-icone download'
					});
		$(Content).html("<div class='ico'></div>baixar");
		DestaqueInferior.append(Content);
		
		
		'a class="acao-icone compartilhar destaque-compartilhar">compartilhar</a>'
		
		var compartilhar = document.createElement('a');
		$(compartilhar).addClass('acao-icone compartilhar destaque-compartilhar');
		$(compartilhar).html("compartilhar");
		
		DestaqueInferior.append(compartilhar);
    ...
    function destaqueSeta(indice) {
      ...
      for (var i = 0; i < destaqueQuantos; i++) {
        ...
        if (i < indice) {
          ...
          var color = eli.attr('data-background');
          eli.find('.destaque-conteudo').css({'background-color': color});
    ...
    /* ################################################################################################################################ */
      $("body").on("click", ".acao-universo", function() {
        
    var destaques = $(".destaque-card");
    
    //var pai = $(this).parents("[content-id]")[0];
    const contentDom = $($(this).parents("[content-id]")[0]);
    
    var salvo = null;

    if (contentDom.find('.proxleitura').hasClass('ativo')) {
      salvo = '1';
    } else {
      salvo = '0';
    }
    ...
    const download = contentDom.attr( 'data-universo-pdf-file' );
    const tell_client = contentDom.attr( 'data-universo-pdf' );
    const tell_client_type = contentDom.attr( 'tell_client_type' );
    const content_url = contentDom.attr( 'content-url' );
    const color = contentDom.attr( 'data-background' );
    ...
    if( download != '' ){
      ...
      $("#destaque").load("pdfiframe.html", function() {
        destaqueMostraConte(1, 1, true, DestaqueInferior, salvo, id);
        $("#pdf-conteudo .loader-panel").removeClass("loader-inactive").append("<span></span>");
        carregaPDF(tell_client, $("#pdf-conteudo"));
      });
      
    } else {
    
      //$(".destaque-item").remove();
    
      var DestaqueItemAtivo = document.createElement('div');
      
      $(DestaqueItemAtivo).addClass('destaque-item ativo');
      $(DestaqueItemAtivo).attr('content-id', id );
      $(DestaqueItemAtivo).attr('content-url', tell_client );
      $(DestaqueItemAtivo).attr('content-download', download );
      $(DestaqueItemAtivo).attr('data-background', color );
    
      //if( tell_client_type == 'pdf' ) $(DestaqueItemAtivo).attr('data-pdfurl', tell_client );
      
      //data-pdfurl="http://localhost/renault/uploads/5198.pdf"
      
        var DestaqueLinkTopo = document.createElement('a');
        
        $(DestaqueLinkTopo).addClass('destaque-linktopo');
        
        $(DestaqueLinkTopo).html("<div class='icone icone-galeria'></div><span>Galeria WhatsApp</span>");
        
        DestaqueItemAtivo.append(DestaqueLinkTopo);
        
        
        /*<div class="destaque-controlevideo">
          <a class="destaque-play"></a>
          <a class="destaque-mute"></a>
         </div>*/
         
         if( tell_client_type == 'video' ){
           
           var ControleVideo = document.createElement('div');
           $(ControleVideo).addClass('destaque-controlevideo');
           $(ControleVideo).html('<a class="destaque-play"></a><a class="destaque-mute"></a>');
            
           
           DestaqueItemAtivo.append(ControleVideo);
        ...
        if(color != 'undefined') {
          $(DestaqueInferior).css({'background-color': color});
        } else {
          $(DestaqueInferior).css({'background-color': '#000000'});
        }
        ...
        var compartilhar = document.createElement('a');
        $(compartilhar).addClass('acao-icone compartilhar destaque-compartilhar');
        $(compartilhar).html("<div class='ico'></div>compartilhar");
        
        DestaqueInferior.append(compartilhar);
        
        DestaqueItemAtivo.append(DestaqueInferior);
        
        
        
        var DestaqueConteudo = document.createElement('div');
        
        $(DestaqueConteudo).addClass('destaque-conteudo');
        
        if(color != 'undefined') {
          $(DestaqueConteudo).css({'background-color': color});
        } else {
          $(DestaqueConteudo).css({'background-color': '#000000'});
        }

    
        var DestaqueDegradeInferior = document.createElement('div');
        
        $(DestaqueDegradeInferior).addClass('destaque-degradeinferior');
        
        DestaqueInferior.append(DestaqueDegradeInferior);
        
        
        var DestaqueTela = document.createElement('div');
        
        $(DestaqueTela).addClass('destaque-tela hammerset');
        $(DestaqueTela).attr({'style': 'touch-action: none; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);'});
        ...
        if( tell_client_type == 'video' ){
          ...
          Media.append(Source);
            ...
            $(Media).addClass('media');
            $(Media).attr({'src': tell_client});
            
            $(Media).html("<div class='destaque-texto'><div class='destaque-texto-solo solo font-16'></div></div>");
          
        }
        
        
        
        DestaqueTela.append(Media);
        
        
        DestaqueConteudo.append(DestaqueTela);
        
        
        DestaqueItemAtivo.append(DestaqueConteudo);
        
      
      $("#destaque").append(DestaqueItemAtivo);
    
    //$(".destaque-item ativo").append(DestaqueItemAtivo);
    
    var indice = destaques.index(this);
    
    
    if (options.isApp) 
      destaqueMostraConte(1, 1);
    else $("#destaque").load("destaques.html", function() {
          destaqueMostraConte(1, 1);
        });
    }
    ...
    /* ################################################################################################################################ */  
	  ...
	  $("body").on("click", ".acao-conte", function() {
		...
		const tell_client = contentDom.attr( 'tell_client' );
		const tell_client_type = contentDom.attr( 'tell_client_type' );
		const content_url = contentDom.attr( 'content-url' );
    const color = contentDom.attr( 'data-background' );
		
		if( tell_client_type == 'pdf' ){
			...
			$("#destaque").load("pdfiframe.html", function() {
			  destaqueMostraConte(1, 1, true, DestaqueInferior);
			  $("#pdf-conteudo .loader-panel").removeClass("loader-inactive").append("<span></span>");
			  carregaPDF(tell_client, $("#pdf-conteudo"));
			});
    ...
    */
	...
	/* CADASTRO FORM */
      ...
      if (!bodyEvents) $('body').on('click', ".login-link-proximo", function(ev){
        ev.preventDefault();
        ev.stopImmediatePropagation();

        if(step == 1) {
          if(!validaNome("input[name='nome']")){
            return false;
          }
        }

        if(step == 2) {
          if(!validaEmail("input[name='email']")){
            return false;
          }
        }

        if(step == 3) {
          if(!validaBIR("input[name='bir']")){
            return false;
          }
        }

        if(step == 4) {
          if(!validaSenhaNova("input[name='senha-nova']") || !validaSenhaConfirmacao("input[name='senha-confirmacao']")){
            return false;
          }
        }
        ...
        if(step < 4){
          $('.step').removeClass('on');
          $('.bull').removeClass('fill');

          step++;

          $('.step-'+step).addClass('on');
          $('.bull-'+step).addClass('fill');

          if(step == 4) {
            $('.login-link-proximo').removeClass('on');
            $('#botao-input-cadastro').addClass('on');
			$('.info-outside').removeClass('on');
            $('.info-outside.final').addClass('on');
          }

          $('.login-back').removeClass('on');
          $('.login-voltar').addClass('on');
        }
      })


      if (!bodyEvents) $('body').on('click', ".login-voltar", function(ev){
        ev.preventDefault();
        ...
        if(step > 1){
          ...
          if(step == 1) {
            $('.login-back').addClass('on');
            $('.login-voltar').removeClass('on');
          }
          
          $('.login-link-proximo').addClass('on');
          $('#botao-input-cadastro').removeClass('on');
		  $('.info-outside').addClass('on');
          $('.info-outside.final').removeClass('on');
