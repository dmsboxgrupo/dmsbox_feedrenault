...
class Feed extends Page {
	...
	parsePostLinks( data ) {
		...
		data.contains_link = data.type == 'campanha' || data.type == 'comunicados' || data.app_link || data.link;
	...
	createHighlight( data ) {
		...
		let html = 
			`<div class="destaque-item ${data.link ? "havelink" : ""}" ${data.header_div}
		...
		if (data.content_type === 'image') {
			...
			html += `<a class="acao-icone editar ${border}"></a>`;

		}

		if (data.favorite !== undefined) {
			
			html += `<a class="acao-icone favoritar ${ data.favorite ? 'ativo' : '' } ${border}"></a>`;

		}
		
		if (data.read_later !== undefined) {
			
			html += `<div class="acao-icone proxleitura ${ data.read_later ? 'ativo' : '' } ${border}"></div>`;
		...
		if (data.download !== undefined) {
			...
			html += `<a ${data.header_div} class="acao-icone download">baixar</a>`;
			
		}
		
		if (data.share_link !== undefined) {
		
			html += `<a class="acao-icone compartilhar destaque-compartilhar">compartilhar</a>`;
	...
	createGallery( data, style='' ) {
		...
		let html = '<div class="galeria-item">';
	...
	createCampaign( data ) {
		...
		if (data.content) {
			...
			html += `<a href="#p=campanhas&id=${data.id}"><img class="imagem-100vw" src="${data.content}" /></a>`;
		...
		if ( data.download !== undefined ) {
			...
			html += `<div class="botao-ico"><div class="acao-icone proxleitura ${ data.read_later ? 'ativo' : '' }"></div><div class="acao-texto ${ data.read_later ? 'ativo' : '' }">${ data.read_later ? 'Salvo!' : 'Salvar' }</div></div>`;
		...
		*/
		...
		dom.find('[data-postid]').click((e) => {
			...
			ui.loadPage('comunicado', '', this.createCampaignModal( data ));
	...
	createPost( data ) {
		...
		let html = 
			`<div class="card" ${data.header_div} ${data.tags_div}>
		...
		if(data.user_score < 3){
		html += `	  <div class="acao-icone curtir"></div>`;
		html += `	  <div class="acao-texto"><span></span> Curtir</div>`;
		} else {
		html += `	  <div class="acao-icone curtir ativo"></div>`;
		html += `	  <div class="acao-texto ativo"><span data-custom="score-count">${data.score_count}</span> ${ data.score_count <= 1 ? 'Curtida' : 'Curtidas' }</div>`;
		}
		if (data.read_later !== undefined) {
		...
		/* ########### BOTÃO "CONTE PARA SEU CLIENTE" ########### */
		if (data.type == 'noticias' && data.download != '') {
			 /*console.log('--------teste--------------');
			 console.log(data);
			 console.log('--------teste fim--------------');*/
		 	html += '<div class="acao-conte"><div class="ico"><img src="images/acaoconte.svg" /></div> conte para <br> seu cliente</div>';
 		 }
		/* ########### BOTÃO "CONTE PARA SEU CLIENTE" ########### */
	...
	createCampaignModal( data, options = { backlink: "javascript:history.back()" } ) {
		...
		let html = 
			`<div class="comunicado-container">`;
				...
				//   <div class="header-title"><img src='images/cabecalho-campanha.svg' /><span>Campanha</span></div>
				//   <a ${options.backlink ? 'href="' + options.backlink + '"' : ''} class="header-icon header-icon-1 comunicado-fechar"><img class="comunicado-fecharimg" src='images/cabecalho-retornar.svg'/></a>
				//   <img class="header-icon header-icon-4" src='images/icorenaultnovo.svg'/>
				// </div>`;  

		html += `<div class="page-content scroll-vertical corpo-comunicado">`;
		html += `<div class="comunicado-fechar-desktop"></div>`;

		if (data.content) {
			
			html += `<img class="imagem-100vw" src="${data.content}" />`
			
		}
		
		html += `<div id="${domGalleryID}" class="galeria galeria-horizontal scroll-horizontal">`;
		html += `</div>`;
		
		if (data.search_galeries) {

			html += `<div id="${domLoadingID}" class="loading-line"><img class="loading" src="images/loading-spinner.svg"/></div>`;
		...
		html += `<div id="footer-comunicado" class="campanha-barra">`
		...
		if ( data.search_galeries !== undefined ) {
			...
			html += `<a href="#p=galeriawhats&search=${data.search_galeries}" class="botao-cinza botao-galeria"></a>`;
		...
		if (data.favorite !== undefined) {
			...
			html += `<a class="acao-icone curtir ${ data.favorite ? 'ativo' : '' } push"></a><div class="acao-texto"><span></span> Curtir</div>`;
	...
	createHandoutModal( data, options = { backlink: "javascript:history.back()" }  ) {
		...
		<div class="header header-fixed header-logo-center header-comunicado">
		  <div class="header-title"><img src='images/cabecalho-comunicado.svg' /><span>${data.origin}</span></div>
		  <a ${options.backlink ? 'href="' + options.backlink + '"' : ''} class="header-icon header-icon-1 comunicado-fechar"><img class="comunicado-fecharimg" src='images/cabecalho-retornar.svg'/></a>
		  <img class="header-icon header-icon-4" src='images/icorenaultnovo.svg'/>
		</div>  
		...
		html += 
		`<a class="acao-icone proxleitura ${ data.read_later ? 'ativo' : '' } push right-15"></a>        
		  <a class="acao-icone favoritar ${ data.favorite ? 'ativo' : '' }"></a>
		</div>`;
	...
	loadSubsession( options = {} ) {
		...
		if (mudandoSubsecao) {
			...
			$("#subsecao").append('<div class="loading-sub loading-line"><img class="loading" src="images/loading-spinner.svg"/></div>');
		...
		const params = Object.assign({}, this.feed, this.feedParams, {q:'feed', filter, type, page, search: searchValue});
		...
		this.controllers.subsession = app.request(params, ( res ) => {
			...
			if (feed.length > 0) {
				...
				if ( type == 'posts' || type == '' ) loadPosts( feed );
